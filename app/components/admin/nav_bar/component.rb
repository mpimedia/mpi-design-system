...
module Admin
  module NavBar
    class Component < ViewComponent::Base
      ...
      # @param environment [Symbol] :development, :staging, :production
      ...
      def section_link_class(section_key)
        base = "mds-navbar__section-link"
        @current_section == section_key ? "#{base} #{base}--active" : base
      end

      def subnav_link_class(subsection_key)
        base = "mds-subnav__link"
        @current_subsection == subsection_key ? "#{base} #{base}--active" : base
      end
      ...
      def show_env_bar?
        @environment && ENVIRONMENT_COLORS.key?(@environment)
      end

      def env_bar_class
        modifier = ENVIRONMENT_COLORS[@environment]
        "mds-env-bar mds-env-bar--#{modifier}"
      end
      ...
      def markaz_logo_svg
        <<~SVG.html_safe
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path d="M4.22 4.22a.75.75 0 0 1 1.06 0L10 8.94l4.72-4.72a.75.75 0 1 1 1.06 1.06L11.06 10l4.72 4.72a.75.75 0 1 1-1.06 1.06L10 11.06l-4.72 4.72a.75.75 0 0 1-1.06-1.06L8.94 10 4.22 5.28a.75.75 0 0 1 0-1.06Z" fill="#{@environment == :staging ? '#DC3545' : '#2E75B6'}"/>
          </svg>
        SVG
      end
