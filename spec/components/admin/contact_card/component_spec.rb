...
RSpec.describe Admin::ContactCard::Component, type: :component do
  let(:default_params) do
    {
      ...
      tags: [
        { label: "Theatrical", color: "#E8733A", bg_color: "#FEF3EC" },
  ...
  it "renders tag pills with group colors" do
    render_inline(described_class.new(**default_params))

    expect(page).to have_css("span[style*='color: #E8733A'][style*='background-color: #FEF3EC']", text: "Theatrical")
    expect(page).to have_css("span[style*='color: #2DA67E'][style*='background-color: #ECF8F4']", text: "Critic")
  end

  it "renders tag pills as rounded pills" do
    render_inline(described_class.new(**default_params))

    expect(page).to have_css("span[style*='border-radius: 999px']", text: "Theatrical")
  end

  it "renders last engaged time" do
    render_inline(described_class.new(**default_params))
