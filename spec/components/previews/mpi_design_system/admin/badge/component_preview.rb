...
class MpiDesignSystem::Admin::Badge::ComponentPreview < ApplicationComponentPreview
  # @label Default
  def default
    render MpiDesignSystem::Admin::Badge::Component.new(label: "Active", color: :primary)
  end
  ...
  def tag_groups
    render_with_template(
      locals: {
        badges: MpiDesignSystem::Admin::TagChip::Component::GROUPS.keys.map do |group|
  ...
  # @label With Count
  def with_count
    render MpiDesignSystem::Admin::Badge::Component.new(label: "Contacts", color: :primary, count: 42)
  end
end
